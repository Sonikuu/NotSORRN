...
void onRender(CSprite@ this)
{
	...
	//hey lets fiddle with new render junk
	
}

void testFunc(CBlob@ this, int id)
{
	Vec2f basepos = this.getPosition();
	Vertex vert1(basepos.x, basepos.y, 0, 0, 30, 0xFFFFFFFF);
	Vertex vert2(basepos.x + 50, basepos.y, 1, 0, 30, 0x0000FFFF);
	Vertex vert3(basepos.x, basepos.y + 50, 0, 1, 30, 0x00FF00FF);
	Vertex[] vertlist(4);
	vertlist[0] = vert1;
	vertlist[1] = vert2;
	vertlist[3] = vert3;
	vertlist[2] = Vertex(basepos.x + 50, basepos.y + 50, 1, 1, 30, 0xFFFFFFFF);
	Render::RawQuads("PixelYellow.png", vertlist);
}
		...
		for (uint i = 0; i < aps.length; i++)
		{
			...
			if (blob !is null && ap.socket && ap.name != "DRIVER" && !this.hasTag("immobile"))
			{
				...
				if(part !is null)
				{
					...
					if(driver !is null && driver.isKeyJustPressed(key_inventory))
						part.onCreateInventoryMenu(this, driver, null);
				}
			}  
		}   
	}
	...
	if (this.getAttachmentPoints(@aps))
	{
		...
		for (uint i = 0; i < aps.length; i++)
		{
			...
			if (blob !is null && ap.socket && ap.name != "DRIVER" && !this.hasTag("immobile"))
			{
				...
				if(part !is null)
				{
					part.onCreateInventoryMenu(this, driver, @gridmenu);
				}
			}  
		}   
	}
