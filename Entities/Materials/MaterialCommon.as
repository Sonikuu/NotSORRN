...
namespace Material
{
	...
	void fromTile(CBlob@ this, uint16 &in type, float &in damage)
	{
		...
		// < 400 means it only does this for vanilla tiles
		...
		{
		  createFor(this, 'mat_sand', 4.f * damage);
		}
		else if(type >= 406 && type <= 408)//Pureified Tiles
		{
		  createFor(this, 'mat_puredust', 10.f * damage);
		}
		else if(type >= 400 && type <= 405)//Corrupt Tiles
		{
		  createFor(this, 'mat_corrpulp', 5.f * damage);
		}
		else if(type >= 409 && type <= 418)//Marble Tiles
		{
			if(XORRandom(2) > 0)
				createFor(this, 'mat_marble', 1.f * damage);
		}
		else if(type >= 428 && type <= 437)//Basalt Tiles
		{
			if(XORRandom(2) > 0)
				createFor(this, 'mat_basalt', 1.f * damage);
		}
