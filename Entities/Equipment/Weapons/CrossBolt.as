...
void onTick(CBlob@ this)
{
	...
	if (!this.hasTag("collided")) //we haven't hit anything yet!
	{
		...
		if (shape.vellen > 0.0001f)
		{
			...
			{
				shape.SetGravityScale(0.1f * this.get_f32("gravity"));
			}
			else
			{
				shape.SetGravityScale(Maths::Min(1.0f, 1.0f / (shape.vellen * 0.1f))  * this.get_f32("gravity"));
			}
...
f32 getArrowDamage(CBlob@ this, f32 vellen = -1.0f)
{
	...
	if (vellen >= arrowMediumSpeed)
	{
		return 1.0f * this.get_f32("damage");
	}

	return 0.5f * this.get_f32("damage");
}
