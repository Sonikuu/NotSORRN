...
CBlob@ server_BuildBlob(CBlob@ this, BuildBlock[]@ blocks, uint index)
{
	...
	if (b.buildOnGround)
	{
		...
		GE_BuildBlob(this.getPlayer().getNetworkID(), b.name);
	}
	...
	if (getNet().isServer())
	{
		...
		if (blockBlob !is null)
		{
			...
			else if(b.name == "coopkey")
			{
				...
				blockBlob.server_setTeamNum(teamnum);
				this.server_setTeamNum(teamnum);
				if(this.getPlayer() !is null)
					this.getPlayer().server_setTeamNum(teamnum);
			}
			if (b.temporaryBlob)
			{
