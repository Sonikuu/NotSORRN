...
float drawScoreboard(CPlayer@[] players, Vec2f topleft, CTeam@ team, Vec2f emblem)
{
	...
	bool isnull = team is null;
	
	GUI::DrawPane(topleft, bottomright, isnull ? SColor(255, 155, 155, 155) : team.color);
	...
	//draw team info
	GUI::DrawText(getTranslatedString(isnull ? "Neutrals" : team.getName()), Vec2f(topleft.x, topleft.y), SColor(0xffffffff));
...
void onRenderScoreboard(CRules@ this)
{
	...
	for (u32 i = 0; i < getPlayersCount(); i++)
	{
		...
		else if (teamNum > 8 || teamNum < 0) //neutral*/
		{
			for (u32 j = 0; j < neutrals.length; j++)
			{
				if (getKDR(neutrals[j]) < kdr)
				{
					neutrals.insert(j, p);
					inserted = true;
					break;
	...
	/*if (localTeam > 8 || localTeam < 0)
		topleft.y = drawScoreboard(neutrals, topleft, this.getTeam(0), Vec2f(0, 0));
	else*/
		topleft.y = drawScoreboard(neutrals, topleft, this.getTeam(1), Vec2f(32, 0));
	topleft.y += 52;
