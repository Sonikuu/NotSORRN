...
float drawScoreboard(CPlayer@ localplayer, CPlayer@[] players, Vec2f topleft, CTeam@ team, Vec2f emblem)
{
	...
	GUI::DrawPane(topleft, bottomright, team is null ? SColor(255, 150, 150, 150) : team.color);
	...
	//draw team info
	GUI::DrawText(getTranslatedString(team is null ? "Neutral" : team.getName()), Vec2f(topleft.x, topleft.y), SColor(0xffffffff));
...
void onRenderScoreboard(CRules@ this)
{
	...
	for (u32 i = 0; i < getPlayersCount(); i++)
	{
		...
		int teamNum = Maths::Min(p.getTeamNum(), 8);
		//if (teamNum == 0) //blue team
		{
			for (u32 j = 0; j < teams[teamNum].length; j++)
			{
				if (getKDR(teams[teamNum][j]) < kdr)
				{
					teams[teamNum].insert(j, p);
					inserted = true;
					break;
				}
			}
	...
	if (localPlayer is null)
		return;
	int localTeam = 0; //localPlayer.getTeamNum();
	...
	for (u32 i = 0; i < teams.length; i++)
	{
		topleft.y = drawScoreboard(localPlayer, teams[i], topleft, this.getTeam(i), Vec2f(0, 0));
		if(teams[i].length > 0)
			topleft.y += 52;
	}
